...
const GuidelineOverlay: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <Info className="h-4 w-4 text-white" /> {/* Changed from text-gray-300 to text-white */}
      </Button>
      ...
      {isVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50 overflow-y-auto p-4 flex flex-col">
          <Button
            variant="ghost"
            className="self-end p-2 text-white hover:text-white/70" // Changed text color to white
            onClick={() => setIsVisible(false)}
          >
            <X className="h-5 w-5" />
          </Button>
          
          <div className="max-w-lg mx-auto text-white text-sm space-y-4 mt-4"> {/* Changed text-gray-200 to text-white */}
            <h2 className="text-xl font-bold text-white mb-4">Game Guidelines</h2>
            
            <div>
              <h3 className="font-bold text-white mb-1">Game Rules:</h3>
              <ul className="list-disc pl-6 space-y-1 text-white"> {/* Added text-white */}
